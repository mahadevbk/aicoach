...
st.markdown("""
    ...
    /* FIX: High-Contrast Sliders */
    .stSlider label p { color: #ccff00 !important; font-weight: 800 !important; font-size: 1.1rem !important; }
    div[data-testid="stThumbValue"] { color: #ffffff !important; background-color: #ccff00 !important; font-weight: 700 !important; }
    div[data-testid="stTickBarMin"], div[data-testid="stTickBarMax"] { color: #94a3b8 !important; }
...
def download_model():
    ...
    if not os.path.exists(model_path):
        ...
        with st.spinner("🚀 INITIALIZING HEAVY AI..."):
...
def render_final_stereo(p1, p2, f1, f2, fps):
    """Safely renders combined video by normalizing heights to 720p."""
    cap1, cap2 = cv2.VideoCapture(p1), cv2.VideoCapture(p2)
    offset = f1 - f2
    target_h = 720
    
    # Calculate widths based on aspect ratio at 720p height 
    w1 = int(cap1.get(cv2.CAP_PROP_FRAME_WIDTH) * (target_h / cap1.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    w2 = int(cap2.get(cv2.CAP_PROP_FRAME_WIDTH) * (target_h / cap2.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    
    temp_out = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
    out = cv2.VideoWriter(temp_out.name, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w1 + w2, target_h))
    ...
    for i in range(total_len):
        ...
        if 0 <= idx2 < cap2.get(cv2.CAP_PROP_FRAME_COUNT):
            ...
            frame2 = np.zeros((int(cap2.get(cv2.CAP_PROP_FRAME_HEIGHT)), int(cap2.get(cv2.CAP_PROP_FRAME_WIDTH)), 3), dtype=np.uint8)

        # Normalize Heights to prevent np.hstack ValueError 
        f1_res = cv2.resize(frame1, (w1, target_h))
        f2_res = cv2.resize(frame2, (w2, target_h))
...
# --- 3. UI LAYOUT (Preserving Original Structure) ---
    ...
    with tabs[i]:
        ...
        with col_render:
            ...
            if is_stereo and f"sync_{sport}" in st.session_state:
                ...
                r1, img1 = cap1.read(); r2, img2 = cap2.read()
                cap1.release(); cap2.release()
                
                if r1 and r2:
                    # Fix colors (BGR to RGB)
                    preview = np.hstack((cv2.resize(cv2.cvtColor(img1, cv2.COLOR_BGR2RGB), (640, 480)), 
                                         cv2.resize(cv2.cvtColor(img2, cv2.COLOR_BGR2RGB), (640, 480))))
                ...
                if st.button("🎬 GENERATE PRODUCTION PACK", key=f"gen_{sport}", use_container_width=True):
                    with st.spinner("Rendering Stereo Video..."):
                        final = render_final_stereo(s['p1'], s['p2'], sl1, sl2, s['d1']['fps'])
                        st.success(f"Production Ready! Offset: {sl1 - sl2}")
                        st.video(final)
                        with open(final, "rb") as f:
                            st.download_button("📥 Download Final Video", f, file_name="stereo_final.mp4")
