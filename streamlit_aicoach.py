...
# --- 2. BIOMECHANIC CALCULATORS (OPTIMIZED FOR CLAUDE) ---
OPTIMIZED_INDICES = [0, 11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27, 28, 29, 30] # Heels Included
...
def analyze_vid(path, model):
    ...
    while cap.isOpened():
        ...
        history.append(lms); raw.append([{"id": j, "x": l.x, "y": l.y, "z": l.z} for j, l in enumerate(lms)] if lms else None)
...
# THE FULL 8-SPORT LIST RESTORED
SPORT_CONFIG = {
    "TENNIS 🎾": ["Serve", "Forehand", "Backhand"],
    "PADEL 🎾": ["Bandeja", "Vibora", "Smash"],
    "PICKLEBALL 🥒": ["Dink", "Third Shot Drop", "Volley"],
    "GOLF ⛳": ["Driver", "Iron", "Putter"],
    "BADMINTON 🏸": ["Smash", "Drop Shot", "Clear"],
    "CRICKET 🏏": ["Drive", "Bowling", "Pull Shot"],
    "GYM 🏋️": ["Squat", "Deadlift", "Bench Press"],
    "YOGA 🧘": ["Warrior", "Tree Pose", "Sun Salutation"]
}
            ...
            if res_key in st.session_state:
                ...
                if st.button("🎬 GENERATE PRODUCTION PACK", key=f"gen_{sport}", use_container_width=True):
                    ...
                    tele_opt = {
                        ...
                        "metadata": {"fps": s['d1']['fps'], "impact_frame": sl1, "offset": int(sl1-sl2)},
